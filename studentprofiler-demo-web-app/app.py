...
def get_students_for_teacher(teacher_uuid):
    ...
    students_collection = db.collection('teachers').document(
        teacher_uuid).collection('students')
...
def get_student_for_teacher(teacher_uuid, student_uuid):
    # Retrieve student details for the selected teacher from Firestore
    student_doc = db.collection('teachers').document(
        teacher_uuid).collection('students').document(student_uuid).get()
    student_data = student_doc.to_dict()
...
def emotional_values():
    ...
    emotion_percentages = calculate_emotional_values(
        request.args.get('videoPath'))
